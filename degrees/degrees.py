...
def load_data(directory):
    ...
    path_people = directory + "/people.csv"
    ...
    # Load movies
    path_movies = directory + "/movies.csv"
    with open(path_movies) as f:
        reader = csv.DictReader(f)
        for row in reader:
            movies[row["id"]] = {
                "title": row["title"],
    ...
    # Load stars
    path_stars = directory + "/stars.csv"
    with open(path_stars) as f:
        reader = csv.DictReader(f)
        for row in reader:
            try:
...
def person_id_for_name(name):
    ...
    if len(person_ids) == 0:
        ...
        try:
            person_id = raw_input("Intended Person ID: ")
